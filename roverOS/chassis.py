...
class Motor:
    ...
    def __init__(self, pin_pwm, pin_a, pin_b, pca_object, encoder_a, encoder_b):
        ...
        self.active = True
        threading.Thread(target=self.__get_velocity).start()
    ...
    def __velocity_PID(self, velocity):#using it's own velocity calculation because of thread sync issues
        self.pidV.setpoint = velocity
        old_position = self.get_position()
        while self.pid_active:
            current_velocity = (self.get_position()-old_position)/self.PID_TIME
            old_position = self.get_position()
...
class Chassis:
    ...
    b2 = Motor(6,4,5,pca2, 'GPIO_AUD0', 'AUD_MCLK')
    ...
    def get_velocities(self):
        return [motor.get_velocity() for motor in self.motors]

    def get_positions(self):
        return [motor.get_position() for motor in self.motors]
...
chassis = Chassis()
try:
    chassis.set_velocity(250)
    for _ in range(20):
        print(chassis.get_velocities())
        time.sleep(1)
finally:
    chassis.shutdown()
